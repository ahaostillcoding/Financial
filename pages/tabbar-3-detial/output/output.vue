	...
	<view class="content">
		<uni-forms class="forms" ref="form" :modelValue="formData" :rules="rules">
			<uni-forms-item label="金额:" name="money">
				<uni-easyinput type="digit" v-model="formData.money" placeholder="请输入支出金额" />
			</uni-forms-item>
		...
		<!-- 提交表单添加数据到数据库并跳转回主页面 -->
		<view>
			<button @click="submit" type="primary">增加</button>
	...
	export default {
		...
		methods: {
			...
			submit() {
				...
				db.collection("output").add(data)//获取数据表的信息
